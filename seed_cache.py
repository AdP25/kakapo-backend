"""
Seeds the proxy's exact_cache with curated Q/A pairs and backfills fake rows
in the `requests` table over the last 30 minutes for dashboard demos.

IMPORTANT: Stop the server before running — it writes to proxy.db directly.
...
from app.core.config import DB_PATH, DEFAULT_MODEL, GPT4O_PRICE
from app.db.database import init_db
from app.services.cache_service import exact_cache_key
from app.services.stats_service import count_tokens
...
SEED_QA: list[tuple[str, str]] = [
    ...
    (
        ...
        "90 days of full request logs with export, and access to tuning controls. You also get SSO via Google or GitHub, a dedicated support "
    ...
    (
        ...
        "dense vector embedding, then compared via cosine similarity against embeddings of previously cached prompts. "
...
def seed_fake_history(conn: sqlite3.Connection, n: int = 12):
    ...
    now = datetime.utcnow()
    statuses = ["miss", "miss", "miss", "exact", "exact"]
...
def main():
    ...
    print(f"[seed] Populating exact_cache with {len(SEED_QA)} Q/A pairs…")
    ...
    conn.close()
    print(f"Seeded {len(SEED_QA)} queries into exact_cache")
